...
# ADL_pos="3M 4M 53"
# cfg=D_ADL, mode=ADL, batch_norm=false
def make_cfg(cfg, targets):
    ...
    target_list = sorted(target_list, reverse=True) # 8 7 6 like
    # reverse so we can insert the ADL in right place
    ...
    if pretrained:
        ...
        if mode == 'ADL':
            ...
            for key in state_dict.keys():
                print(key)
            make_config_align(state_dict, cfgs[cfg[0]], cfgs[cfg]) # cfg[0] is 'D', alias vgg16
            print("After config change")
            for key in state_dict.keys():
                print(key)
            print("\n")
            # Above line exacly do nothing...
            strict_rule = False
        elif mode == 'GAP':
            state_dict = remove_layer(state_dict, 'classifier.')
            strict_rule = False
